...
#+END_SRC
here lazyunit is a derived combinator
- it knows nothing about the unit combinator. but knows it will need to use it whatever form it is finally implemented in

*** we evaluate on [get] :drill:
- this allows us to delay plugging in the execution strategy
- when designing a parallel api we must decide on WHEN a computation is evaluated
- it seems to make sense to delay it so that it can be plugged into some form of execution strategy at a later stage 
  - see java executor framework for example
- however we are really tailor fitting our concurrency api to meet our problem domain
- get (now renamed to run)can be designed to take on an executor service and can handle cancellation , interrupption internally or can delegate by returning a future
