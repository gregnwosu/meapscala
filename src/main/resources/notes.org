...
abstract class  Par[A] {
  ...
  def fork[A](a: => Par[A]): Par[A]
  def lazyUnit[A](a: => A): Par[A] = fork(unit(a))
  def run[A](ExecutorService:es)(a: Par[A]): A
}
#+END_SRC

but we have defined the model (form /shape) of Par[A]!!!!
we have defined a class or group fo functions that take or return Par[A] but
we havent said what the type of Par[A] is 
its at this moment we can decide
the simplest model/type/shape/form for Par[A]

#+BEGIN_SRC scala
type Par[A] = ExecutorService => Future[A]
#+END_SRC
- we dont acutally need for Par[A] to take an A value since, the A is passed to its constructors
  - unit(A:a)
  - lazyUnit(A:a)
- the fact that Par[A] is a function type means that the future isnt created until it is supplied with an ExecutorService
